...
namespace DwfClient
{
    class Program
    {
        ...
        static string dwfFile { get; set; }
        static string jpgFile { get; set; }
        static void Main(string[] args)
        {
            dwfFile = args[0]; jpgFile = dwfFile.Replace(".dwf", ".jpg");
            pkgInfo.IsHeader = true;
            pkgInfo.Length = pkgHeaderSize;
            if (client.Connect("192.168.2.18", 1139) == true)
            {
                using (var fs = new FileStream(dwfFile, FileMode.Open))
                {
                    // 封包体
